...
const utilities_1 = require("../../global/scripts/helpers/utilities");
class Navigation {
    constructor() {
        ...
        this.transitionEvent = (0, utilities_1.whichTransitionEvent)();
    ...
    mobileMainNavTrapTabs(e) {
        const elemObj = (0, utilities_1.getFocusableElementBySelector)(this.mainNavElement.id, ['> div button', '> ul > li > a']);
        (0, utilities_1.trapTabKey)(e, elemObj);
    }
    ...
    mobileShowMainNav({ propertyName }) {
        if (!propertyName === 'transform')
            return;
        (0, utilities_1.getFocusableElementBySelector)(this.mainNavElement.id, ['> div button', '> ul > li > a']).all[1].focus();
    ...
    showSubNav({ propertyName }) {
        ...
        if (!propertyName === 'transform')
            return;
        (0, utilities_1.getFocusableElementBySelector)(submenu.id, ['> div button', '> .nsw-main-nav__title a', '> ul > li > a']).all[2].focus();
    ...
    trapkeyEventStuff(e) {
        const { submenu } = this.whichSubNavLatest();
        const elemObj = (0, utilities_1.getFocusableElementBySelector)(submenu.id, ['> div button', '> ul > li > a']);
        (0, utilities_1.trapTabKey)(e, elemObj);
    }
}
